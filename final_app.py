...
history = load_history().dropna(subset=["timestamp"])
...
# --- Simulation ---
    ...
    current["temp"] = st.slider("Temperature (°C)", 20.0, 50.0, float(current["temp"]), 0.1)
    current["pressure"] = st.slider("Pressure (psi)", 60.0, 120.0, float(current["pressure"]), 0.1)
    current["vibration"] = st.slider("Vibration (g)", 0.0, 1.5, float(current["vibration"]), 0.01)
    current["gas"] = st.slider("Gas (ppm)", 0.0, 10.0, float(current["gas"]), 0.01)
...
# --- Predictive Analysis ---
    ...
    future = pd.DataFrame({
        ...
        "pressure": np.linspace(history.pressure.iloc[-1], history.pressure.iloc[-1] + random.uniform(-5,5), 72),
        "vibration": np.linspace(history.vibration.iloc[-1], history.vibration.iloc[-1] + random.uniform(-0.2,0.2), 72),
        "gas": np.linspace(history.gas.iloc[-1], history.gas.iloc[-1] + random.uniform(-1,1), 72)
    })
    fig3 = go.Figure()
