...
def darken(hex_color: str, amount: float = 0.1) -> str: import colorsys c = hex_color.lstrip('#') r, g, b = [int(c[i:i+2], 16) for i in (0,2,4)] h, l, s = colorsys.rgb_to_hls(r/255, g/255, b/255) l = max(0, l - amount) r2, g2, b2 = colorsys.hls_to_rgb(h, l, s) return f"#{int(r2255):02X}{int(g2255):02X}{int(b2*255):02X}"
...
menu = st.sidebar.radio( 'Menu', ['Dashboard','Simulation','Predictive Analysis','Smart Solutions','Settings','About'] )
...
elif menu == 'Simulation': st.markdown("<div class='main-title'>🎛️ Simulation</div>", unsafe_allow_html=True) sd = st.session_state.sim_data or fetch_data() sd['temp'] = st.slider('Temperature (°C)', 0, 100, int(sd['temp'])) sd['pressure'] = st.slider('Pressure (kPa)', 0, 200, int(sd['pressure'])) sd['vibration'] = st.slider('Vibration (mm/s)', 0, 5, float(sd['vibration'])) sd['gas'] = st.slider('Gas (ppm)', 0, 10, float(sd['gas'])) st.session_state.sim_data = sd st.map(pd.DataFrame({'lat':[24.7],'lon':[46.7]})) st.write(sd)
...
elif menu == 'Smart Solutions': st.markdown("<div class='main-title'>🛠️ Smart Solutions</div>", unsafe_allow_html=True) sol = generate_solution(st.session_state.lang) if st.button('Generate Solution'): st.table(sol)

elif menu == 'Settings': st.markdown("<div class='main-title'>⚙️ Settings</div>", unsafe_allow_html=True) lang = st.radio('Language', ['English','العربية']) st.session_state.lang = 'en' if lang=='English' else 'ar' palette = st.selectbox('Color Palette', list(PALETTES.keys())) st.session_state.palette = palette sensitivity = st.slider('Anomaly Sensitivity', 0.01, 0.3, st.session_state.contamination, 0.01) st.session_state.contamination = sensitivity
