...
# ========== النموذج الفيزيائي البسيط للمضخة ==========
if page == "النموذج الفيزيائي":
    st.markdown("<h1 class='section-title'>تمثيل مرئي للنظام الفيزيائي</h1>", unsafe_allow_html=True)
    st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/7/76/Oil_Pump_Jack_Diagram.svg/1024px-Oil_Pump_Jack_Diagram.svg.png",
             caption="تمثيل تخطيطي لمضخة نفط", use_column_width=True)
    st.markdown("هذا الشكل يمثل المكون الفيزيائي المرتبط بالتوأم الرقمي، ويعرض الآليات المرتبطة بجمع البيانات وتحليلها.")

# ========== صفحة تفاعلية للمستخدم لتجربة القيم ==========
elif page == "محاكاة تفاعلية":
    st.markdown("<h1 class='section-title'>محاكاة تفاعلية لسيناريوهات المستشعرات</h1>", unsafe_allow_html=True)
    st.markdown("قم بتغيير قيم المستشعرات وشاهد كيف تتغير درجة المخاطرة والتوصيات في الوقت الفعلي.")

    temp_input = st.slider("درجة الحرارة (°F)", min_value=60, max_value=120, value=int(latest_data_point['temperature']))
    pressure_input = st.slider("الضغط (hPa)", min_value=850, max_value=1050, value=int(latest_data_point['pressure']))
    vibration_input = st.slider("الاهتزاز", min_value=0.0, max_value=1.5, value=float(latest_data_point['vibration']), step=0.01)
    flow_input = st.slider("معدل التدفق", min_value=0.0, max_value=100.0, value=30.0, step=1.0)

    score = calculate_risk_score(temp_input, pressure_input, vibration_input)
    risk_level, color = get_risk_level(score)

    st.plotly_chart(create_gauge_chart(score, "درجة المخاطرة (محاكاة)", colors), use_container_width=True)
    st.markdown(f"**مستوى المخاطرة:** <span style='color:{color}; font-weight:bold;'>{risk_level}</span>", unsafe_allow_html=True)

    user_sensor_input = {
        'pressure': pressure_input,
        'temperature': temp_input,
        'flow_rate': flow_input
    }

    sim_rec = suggest_enhanced_intervention(user_sensor_input, score)
    st.markdown("### توصية استنادًا إلى القيم المُدخلة")
    st.markdown(f"<div class='alert alert-{sim_rec['alert_type']}'>", unsafe_allow_html=True)
    st.markdown(f"**{sim_rec['base_intervention']}**")
    for detail in sim_rec['details']:
        st.markdown(f"- {detail}")
    st.markdown("</div>", unsafe_allow_html=True)